...
import {
  ...
  ChevronDown, Save, Send, Check, Clock,
} from "lucide-react";
      ...
      <div className="space-y-6 max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button onClick={() => step > 1 ? setStep(2) : undefined} className={cn("flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors", step === 2 ? "bg-primary text-primary-foreground" : step > 2 ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-muted text-muted-foreground")}>
            ...
            Items ({items.filter((i) => i.question.trim()).length})
          </button>
          <div className="h-px w-6 bg-border" />
          ...
          <>
            <Card>
              <CardContent className="p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-lg font-bold">{name || "Untitled Checklist"}</h2>
                ...
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-muted-foreground">{items.filter((i) => i.question.trim()).length} items</span>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-2">
                {items.filter((i) => i.question.trim()).map((item, idx) => (
                  <div key={item.id} className="flex items-start gap-3 text-sm py-2">
                    <span className="shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">{idx + 1}</span>
                    <div className="flex-1">
