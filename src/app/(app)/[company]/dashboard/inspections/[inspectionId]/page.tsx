...
export default function InspectionDetailPage() {
  ...
  if (!inspection) {
    return <EmptyState title="Inspection not found" description="The requested inspection could not be found." />;
  }

  const resultVariant = inspection.result === "pass" ? "success" : inspection.result === "fail" ? "destructive" : "warning";
  const resultLabel = inspection.result === "pass" ? "Passed" : inspection.result === "fail" ? "Failed" : "Needs Attention";
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-semibold">{asset?.name || "Inspection"}</h1>
              <Badge variant={resultVariant}>{resultLabel}</Badge>
            </div>
            <p className="text-sm text-muted-foreground mt-1">
              {asset?.asset_tag ? `${asset.asset_tag} • ` : ""}Inspected {formatDate(inspection.inspected_at)}
            </p>
            <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
              <span className="flex items-center gap-1">
                <User className="h-3 w-3" /> {inspector?.full_name || "Unknown Inspector"}
              </span>
              <span className="flex items-center gap-1">
      ...
      <Card className={inspection.result === "pass" ? "border-success/50 bg-success/5" : inspection.result === "fail" ? "border-destructive/50 bg-destructive/5" : "border-warning/50 bg-warning/5"}>
        <CardContent className="py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-medium">Inspection {resultLabel}</p>
              <p className="text-sm text-muted-foreground">
                Result: {inspection.result.replace("_", " ")}
              </p>
            </div>
          </div>
        </CardContent>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <p className="font-medium flex items-center gap-2 mt-1">
                      <Wrench className="h-4 w-4" /> {asset?.name || "Unknown Asset"}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Inspector</p>
                    <p className="font-medium flex items-center gap-2 mt-1">
                      <User className="h-4 w-4" /> {inspector?.full_name || "Unknown"}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Inspected At</p>
                    <p className="font-medium flex items-center gap-2 mt-1">
                  ...
                  {inspection.checklist_id && (
                    <div>
                      <p className="text-sm text-muted-foreground">Checklist ID</p>
      ...
      {activeTab === "details" && (
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Full Inspection Record</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  <div className="p-4 rounded-lg bg-muted/50">
                    <p className="text-xs text-muted-foreground mb-1">Inspection ID</p>
                  ...
                  <div className="p-4 rounded-lg bg-muted/50">
                    <p className="text-xs text-muted-foreground mb-1">Inspector ID</p>
                    <p className="font-mono text-sm">{inspection.inspector_id}</p>
                  </div>
                  {inspection.checklist_id && (
                    <div className="p-4 rounded-lg bg-muted/50">
                      <p className="text-xs text-muted-foreground mb-1">Checklist ID</p>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base flex items-center gap-2">
                  <Image className="h-4 w-4" /> Media Attachments
                </CardTitle>
              </CardHeader>
              <CardContent>
